...
async function proxyRequest(
  ...
  const { apiUrl, adminApiKey } = getServerConfig();

  if (!adminApiKey) {
    console.error("❌ ADMIN_API_KEY is not set in environment variables");
    return NextResponse.json(
      { error: "Admin API key not configured. Please set ADMIN_API_KEY in .env.local" },
      { status: 500 }
    );
  }

  const targetPath = path.join("/");
  const targetUrl = new URL(`/admin/${targetPath}`, apiUrl);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        console.error("❌ 401 Unauthorized - Check ADMIN_API_KEY matches backend configuration");
      }
