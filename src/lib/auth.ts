...
export async function verifyPassword(password: string): Promise<boolean> {
  ...
  if (!passwordHash) {
    ...
    console.error("💡 Run: node scripts/generate-hash.js YOUR_PASSWORD");
    console.error("💡 Then add ADMIN_PASSWORD_HASH to your .env.local file");
    console.error("💡 Make sure the file is named .env.local (not .env)");
    return false;
  }
  ...
  if (!passwordHash.startsWith("$2")) {
    if (process.env.NODE_ENV === "development") {
      ...
      console.error("💡 TIP: Wrap the hash in quotes in .env.local: ADMIN_PASSWORD_HASH='$2b$10$...'");
    }
    return false;
  }
